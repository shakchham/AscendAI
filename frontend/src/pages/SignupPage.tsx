...
export function SignupPage() {
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-slate-950 px-4">
      <div className="absolute -left-20 top-10 h-80 w-80 rounded-full bg-blue-600/20 blur-3xl" />
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-white/15 bg-white/10 p-7 shadow-2xl backdrop-blur-xl">
        ...
        <p className="mb-5 text-sm text-slate-300">Set up your account to start your test prep journey.</p>
        <div className="space-y-3">
          ...
          />
          <button disabled={loading} className="w-full rounded-lg bg-blue-600 px-4 py-2.5 font-medium text-white transition hover:bg-blue-500 disabled:opacity-60" onClick={() => void signUp()}>
            {loading ? "Creating..." : "Sign Up"}
          </button>
        </div>
        <p className="mt-4 text-sm text-slate-200">
          Already have account? <Link className="text-blue-300 underline" to="/login">Login</Link>
        </p>
      </div>
    </main>
  );
